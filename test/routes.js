...
tape("should get echo and reverse", function (t) {
  // Test for echo route
  var echoUrl = "/echo?one=1&two=2";
  servertest(server(), echoUrl, { encoding: "json" }, function (err, res) {
    t.ifError(err, "no error");
    t.equal(res.statusCode, 200, "correct statusCode");
    t.deepEqual(
      res.body,
      { one: "1", two: "2" },
      "values should match for echo"
    );
  });

  // Test for reverse route
  var expected = {
    input: "stringtoreverse",
    output: "esreverotgnirts",
  };
  var reverseUrl = "/reverse/" + expected.input;
  servertest(server(), reverseUrl, { encoding: "json" }, function (err, res) {
    t.ifError(err, "no error");
    t.equal(res.statusCode, 200, "correct statusCode");
    t.deepEqual(res.body, expected, "values should match for reverse");
  });
